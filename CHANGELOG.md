...
* **core** The RE2 engine which bounds `regex` list filters on SQLite and SQL.js is now the pure-JavaScript `re2js` rather than the native `re2`. `re2js` is a plain dependency of `@vendure/core`, so it arrives with the package and nothing has to compile at install time. `re2` was an optional dependency which needed `node-gyp`, plus a build-script allowlist entry on pnpm, yarn and bun. No action is needed on upgrade, and the protection no longer depends on the project installing anything itself ([GHSA-jgm3-qmp2-c4p7](https://github.com/advisories/GHSA-jgm3-qmp2-c4p7)).
