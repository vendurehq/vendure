...
import { CliLogLevel, DbType, LintTool } from './types';
...
export function getLintDependencies(lintTool: LintTool, vendurePkgVersion = ''): string[] {
    switch (lintTool) {
        case 'eslint':
            return [
                'eslint',
                ...
                'typescript-eslint',
                'globals',
                `@vendure/eslint-plugin${vendurePkgVersion}`,
            ];
        case 'biome':
        ...
        case 'none':
            return [];
        default:
            const n: never = lintTool;
            log(pc.red(`No lint dependencies configured for "${n as string}"`));
            return [];
    }
}
