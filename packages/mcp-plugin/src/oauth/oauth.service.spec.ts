...
describe('McpOauthService storefront channel selection', () => {
    function createStorefrontService(activeChannelId: number | undefined) {
        const ctx = {} as any;
        const sessionService = {
            getSessionFromToken: vi.fn().mockResolvedValue({ user: { id: 7 }, activeChannelId }),
        };
        const channelService = {
            getChannelFromToken: vi.fn().mockResolvedValue({ id: 11 }),
        };
        const service = new McpOauthService(
            undefined as any,
            { create: vi.fn().mockResolvedValue(ctx) } as any,
        ...
        const completeAuthorizationRequest = vi
            .spyOn(service as any, 'completeAuthorizationRequest')
            .mockResolvedValue({ redirectUrl: 'https://example.com/cb?code=test' });
        return { channelService, completeAuthorizationRequest, ctx, service };
    }

    it('inherits the authenticated Vendure session channel when the callback omits channelToken', async () => {
        const { channelService, completeAuthorizationRequest, service } = createStorefrontService(9);

        await service.completeStorefrontRequest({
            session: 'authorization-request',
            vendureAuthToken: 'vendure-session',
            approved: true,
        });

        expect(channelService.getChannelFromToken).not.toHaveBeenCalled();
        expect(completeAuthorizationRequest).toHaveBeenCalledWith(
    ...
    it('gives an explicit callback channelToken precedence over the Vendure session channel', async () => {
        const { channelService, completeAuthorizationRequest, ctx, service } = createStorefrontService(9);

        await service.completeStorefrontRequest({
            session: 'authorization-request',
            vendureAuthToken: 'vendure-session',
            channelToken: 'explicit-channel',
            approved: true,
        });

        expect(channelService.getChannelFromToken).toHaveBeenCalledWith(ctx, 'explicit-channel');
        expect(completeAuthorizationRequest).toHaveBeenCalledWith(
            'authorization-request',
            true,
            7,
