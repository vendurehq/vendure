...
describe('McpPlugin OAuth end-to-end flow', () => {
    ...
    // The HTTP shape of the OAuth routes: metadata, form bodies, RFC error bodies, cache headers.
    describe('routes', () => {
        ...
        it('GET /.well-known/oauth-protected-resource/mcp/bogus returns 404', async () => {
            const port = config.apiOptions.port;
            const res = await fetch(
                `http://localhost:${port}/.well-known/oauth-protected-resource/mcp/bogus`,
            );
            expect(res.status).toBe(404);
        });
    });

    // Single-use authorization codes and refresh-token rotation, driven through McpOauthService
    // directly rather than over HTTP.
    describe('single-use code and refresh rotation', () => {
