...
/** One (Role, Channel) grant in the form value. */
...
/**
 * Normalizes form-value pairs into the shape the `roleAssignments` creation input takes. A
 * row mid-edit (no Role picked yet, or a Role with no Channel) is not an assignment and must
 * not reach the input.
 */
...
/**
 * Edits the role assignments of a User being created, as one row per Role listing the
 * Channels that Role is granted on. The form value is the flat list of (roleId, channelId)
 * pairs taken by the `roleAssignments` creation input.
 *
 * Only the Roles and Channels the active user may grant are offered, mirroring the server's
 * `RoleService.canGrant` (the actor holds every permission of the Role on that Channel); the
...
export function RoleAssignmentsEditor({ value, onChange }: Readonly<RoleAssignmentsEditorProps>) {
    ...
    // in the UI. External value changes (form reset) are therefore detected by comparing
    // against the pairs last emitted rather than mirrored on every render.
