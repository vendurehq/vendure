---
title: 'Migrating Dashboard extensions to Design System v2'
metaTitle: 'Vendure Dashboard Design System v2 Migration Guide'
metaDescription: 'Update Vendure Dashboard extensions for Design System v2 component, theming, and markup changes.'
---

# Dashboard Design System v2 migration
...
This guide records the extension-facing changes introduced by the Design System
v2 migration. Each entry explains who is affected and how to update an extension.
...
<StatusBadge tone={orderStateDictionary.toneFor(order.state)}>{order.state}</StatusBadge>;
```
...
> `critical`. Routine and healthy states such as `PaymentSettled`, `Shipped`,
> and `Delivered` are also deliberately neutral; color is reserved for states
> that need attention or communicate exceptional progress.
...
The legacy `--color-vendure-brand` Tailwind theme variable is also no longer
emitted. Use `--color-brand`, backed by the `--brand` theme override, instead.
...
unchanged**. The row-selection and bulk-action _treatment_ now follows the
...
- The old free-floating toolbar row and separately-boxed table wrapper are gone.
  There is no `[data-slot="card"]` wrapper _around_ the table anymore — the card
  root **is** the table.
...
for booleans where `true` is _not_ the expected default, or where the green
...
## 11. `LinkDialog` removed from the public API

The rich text editor now edits links through an inline bubble menu, and the old
modal `LinkDialog` component is no longer exported from `@vendure/dashboard`.

**Affected:** extensions importing and rendering `LinkDialog` directly.

**Migrate:** use `RichTextEditor` for the complete editing experience. Extensions
that provide a custom editor surface can compose the exported
`LinkBubbleMenu` component instead of opening the removed modal dialog.
