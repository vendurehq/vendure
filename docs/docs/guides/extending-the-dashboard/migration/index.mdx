...
:::info[Already using the React Dashboard?]
See the [Design System v2 migration guide](/extending-the-dashboard/migration/design-system-v2/)
for the component, theming, and markup changes that affect existing Dashboard extensions.
:::
...
{/* Note: the following code block should not be edited. It is auto-generated from the files in the
     `skills/vendure-dashboard-migration` dir, by running the npm script `generate-migration-prompt` from
     the `./docs` dir. */}
````md
## Instructions
...
- new code imports all components from `@vendure/dashboard`. This includes re-exports of common third-party utilities:
    - `useQuery`, `useMutation`, `useQueryClient` etc. (TanStack Query)
    - `Link`, `useNavigate`, `useRouter` etc. (TanStack Router)
    - `useForm`, `useFormContext`, `Controller` etc. (React Hook Form)
    - `toast` (Sonner)
- The following are imported from their own packages:
...
Tailwind + Vendure Design System. Components import from `@vendure/dashboard`
...
```ts
import { useQuery, api } from '@vendure/dashboard';
import { graphql } from '@/gql';
...
import { Button, Card, CardContent, cn, DashboardFormComponent, Input, useFormContext } from '@vendure/dashboard';
...
If you use an AI coding agent with [Agent Skills](https://agentskills.io), you can set up a specialized skill for migrating plugins.
This has the advantage that you do not need to continually paste in the full prompt, and it can also be potentially more token-efficient.

To set up the skill, run this from the root of your project:

```
npx skills add vendurehq/vendure --skill vendure-dashboard-migration
```

This installs the `vendure-dashboard-migration` skill from the Vendure repository into your selected agent's skills directory.

You can then have your coding agent use the skill with a prompt like:
...
| `FormGroup` | `useForm` | `@vendure/dashboard` | Form state management |
...
| - | `useFormContext()` | `@vendure/dashboard` | Access field state and errors |
