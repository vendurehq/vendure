...
When you only need to condition an entry you own, `isVisible` is the shorthand. It is available on nav sections and on route `navMenuItem` entries, and it is ANDed with `requiresPermission`: both must pass for the entry to appear.
