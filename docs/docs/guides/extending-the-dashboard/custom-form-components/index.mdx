...
metaTitle: 'Customizing Forms in the Vendure Dashboard'
metaDescription: 'Choose the right API for replacing native detail-page inputs or rendering custom fields with your own Vendure Dashboard input components.'
---
...
| ---------------------------------------------------------------------------- | ------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| Replace the rendered input for a native entity field, such as `Product.slug` | [`detailForms[].inputs`](#replacing-native-detail-page-fields)            | Target a specific `pageId`, `blockId`, and `field`                                   |
| Use my own input for a plugin-defined custom field                           | [`customFormComponents.customFields`](#custom-field-components)           | Register a component ID, then reference it from the custom field's `ui.component`    |
| Use my own input for a configurable operation argument                       | [`customFormComponents.customFields`](#configurable-operation-components) | Register a component ID, then reference it from the argument config's `ui.component` |

Both APIs use the same [DashboardFormComponent type](/reference/dashboard/extensions-api/form-components#dashboardformcomponent), but they are registered differently. Use `detailForms[].inputs` only when you are replacing a native field on a specific detail page. Use `customFormComponents.customFields` when the field itself is defined in your plugin configuration.
...
- **`summary`**: A component rendering a compact, read-only summary of the field's current value. Used to label the value chips when your component is used for a [configurable operation argument](#configurable-operation-components). It may use hooks and data fetching, e.g. to resolve stored IDs to display names.
- **`popoverSize`**: When your component is used for a configurable operation argument, it renders inside a popover. Set to `'lg'` if it needs more room than the default width.

```tsx
import { DashboardFormComponentProps } from '@vendure/dashboard';
...
### How operation arguments are rendered

Configurable operations render as a one-line sentence based on the operation's description template, where
each argument appears as an inline value chip. Clicking a chip opens a popover containing the argument's
registered form component — your component receives exactly the same props as in any other context, so no
changes are needed for it to work there.

Two optional `metadata` properties let you refine how your component behaves in this context:

- Without a `summary`, the chip label falls back to the raw value (or a count for list values). Provide a
  `summary` component to render something more meaningful — this matters especially for components that
  store entity IDs, since the summary can resolve them to display names.
- If your component needs more horizontal space than the default popover provides, set `popoverSize: 'lg'`.

```tsx title="src/plugins/my-plugin/dashboard/components/color-picker.tsx"
ColorPickerComponent.metadata = {
    summary: ({ value }) => (
        <span className="inline-flex items-center gap-1.5">
            <span className="inline-block h-3 w-3 rounded-sm border" style={{ backgroundColor: value }} />
            {value}
        </span>
    ),
    popoverSize: 'lg',
...
For plugin-defined custom fields, use [custom field components](#custom-field-components) instead.
...
- **[Input component examples](/extending-the-dashboard/custom-form-components/form-component-examples)** - Detailed examples of how to use the APIs available for custom form components.
- **[Relation selector components](/extending-the-dashboard/custom-form-components/relation-selectors)** - Build powerful entity selection components with search, pagination, and multi-select capabilities for custom fields and form inputs
