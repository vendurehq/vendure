...
Use this API when the existing detail page is mostly right and you only need to add data, replace a field input, or support custom page blocks. You usually do not need to recreate the whole detail route.
...
Use `detailForms.inputs` only when you want to replace the input for a specific built-in field. For custom fields, use [custom field components](/extending-the-dashboard/custom-form-components/#custom-field-components).
