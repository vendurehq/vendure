...
You can customize the dashboard theme colors by modifying the `theme` configuration in your `vite.config.mts` file. The `primary` tokens style primary actions, while the `brand` tokens are reserved for identity moments such as logos and branded accents.
    ...
    vendureDashboardPlugin({
      ...
      theme: {
        ...
        dark: {
          ...
          // Brand colors can be tuned independently for dark mode
          brand: "oklch(0.65 0.16 240)",
          "brand-foreground": "oklch(0.12 0.03 240)",
        },
      },
...
| `--brand-foreground` | Content displayed on a brand-colored surface |
