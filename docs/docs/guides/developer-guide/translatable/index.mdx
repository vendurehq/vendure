...
:::note
Vendure identifies a translation entity by it being the target of a `translations` relation. An entity wired up as shown above is therefore always identified as a translation entity, and Vendure will not create a `config.customFields` entry for it. Only the base entity gets one.

A name ending in `Translation` and a `languageCode` field are not enough on their own. If no other entity points a `translations` relation at it, such an entity is not treated as a translation entity, and it gets a `config.customFields` entry like any other entity that declares custom fields.
:::
