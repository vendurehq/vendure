...
:::note
Custom fields are auto-initialized for every entity that declares a `customFields` embedded, **except** translation entities, which are detected by being the *target of a `translations` relation* — not by a `*Translation` name suffix or a `languageCode` column. In practice a properly-wired translation entity (as above) is always excluded. The only behavioural edge case: an entity that merely *looks* like a translation entity by name and `languageCode` but has no `translations` relation pointing at it is now treated as a normal custom-field-bearing entity and seeded, whereas earlier it was excluded by the old name+column heuristic.
:::
