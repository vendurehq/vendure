...
environment variable. A schedule of `0 3 * * *` therefore means "03:00 in whatever timezone the process happens
to run in" - which for a process running in UTC is 03:00 UTC, not 03:00 local time.
...
For tasks you do not construct yourself, such as the built-in `cleanSessionsTask`, the timezone can be set
via the `configure()` method:

```ts
cleanSessionsTask.configure({ timezone: 'Europe/Stockholm' });
```

With a timezone configured, schedules follow local wall-clock time including daylight saving time transitions,
independently of the process timezone. This is particularly useful when the process runs in UTC (which is
generally recommended so that database timestamps are stored consistently), but tasks should still run at a
fixed local time.

When no timezone is configured, the behaviour is unchanged from prior versions.
