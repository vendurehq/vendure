...
With a timezone configured, schedules follow local wall-clock time, including daylight saving time transitions,
whatever the process timezone is. This lets the process run in UTC, which keeps database timestamps consistent,
while tasks still run at a fixed local time.

An identifier the runtime does not recognise fails at startup with an error naming the option or task that
carries it.
