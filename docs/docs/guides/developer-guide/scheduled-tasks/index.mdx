...
By default, cron expressions are evaluated in the timezone of the Node.js process, as determined by the `TZ`
environment variable. A schedule of `0 3 * * *` means "03:00 in whatever timezone the process runs in". For a
process running in UTC, that is 03:00 UTC rather than 03:00 local time.

Since v3.8 you can specify an [IANA timezone identifier](https://en.wikipedia.org/wiki/List_of_tz_database_time_zones)
in which schedules should be evaluated, either globally or per task:

```ts title="vendure-config.ts"
import { ScheduledTask, VendureConfig } from '@vendure/core';

export const config: VendureConfig = {
    // ...
    schedulerOptions: {
        // All schedules are evaluated in this timezone...
        timezone: 'Europe/Stockholm',
        tasks: [
            new ScheduledTask({
                id: 'nightly-sync',
                schedule: cron => cron.everyDayAt(2, 0),
                // ...except for tasks that specify their own
...
For tasks you do not construct yourself, such as the built-in `cleanSessionsTask`, use the `configure()` method
where the task is added to the config:

```ts title="vendure-config.ts"
export const config: VendureConfig = {
    // ...
    schedulerOptions: {
        tasks: [cleanSessionsTask.configure({ timezone: 'Europe/Stockholm' })], // [!code highlight]
    },
};
```

With a timezone configured, schedules follow wall-clock time in that timezone, including daylight saving time
transitions. The process timezone no longer affects when a task runs. The process can therefore stay on `TZ=UTC`,
which is what Vendure expects for correct timestamp persistence, while tasks run at a fixed local time.

An invalid identifier throws at startup. The error names the option or task holding it.
