---
title: 'Custom Fields'
metaTitle: "Adding Custom Fields to Vendure Entities"
metaDescription: "Extend Vendure entities like Product, Customer, and Order with custom data fields. Covers field types, validation, UI configuration, and relational fields."
order: 3
---
...
Whether a database index should be created for the custom field. Use this option for fields which
are frequently filtered or sorted in large data sets. It is supported for scalar and single relation
fields, but not for list, struct, or secret fields. MySQL and MariaDB do not support this option for
`text` or `localeText` fields because those values are stored as `longtext` without an index prefix
length.

Adding `index: true` to an existing custom field changes the database schema. Generate and run a
[database migration](/guides/developer-guide/migrations/) after updating the configuration.
...
The [onDelete](https://typeorm.io/docs/relations/relations/) behavior for the relation. If not set, the database default (`NO ACTION`) applies.

:::caution

Setting `onDelete: 'CASCADE'` means that when the referenced entity is deleted, the database also deletes every row that references it through this custom field. For example, with a relation to `ProductVariant` and `onDelete: 'CASCADE'`, deleting a `ProductVariant` also deletes every row that references it. These deletions happen at the database level, so no Vendure events are published for the deleted rows.

Setting `cascade: ['remove']` acts in the opposite direction: removing the owning entity with TypeORM's `remove()` also removes the related entity. The `cascade` setting has no effect when the referenced entity is deleted.

:::

Setting `onDelete` on a `ManyToMany` (`list: true`) relation applies to the foreign keys of the join table: deleting the related entity removes the join-table rows, and the rows of both entities are left untouched.
...
Setting `onUpdate` on a `ManyToMany` (`list: true`) relation applies to the foreign keys of the join table, not to the related entity.
