...
Encryption requires a stable secret key, provided via the `VENDURE_ENCRYPTION_KEY` environment
variable (or by passing a `secret` to the [`DefaultEncryptionStrategy`](/reference/typescript-api/configuration/encryption-strategy/)):
...
If a `secret` field or arg is configured but no key is available, the server will refuse to start.
...
which you can replace to apply your own logic (for example, revealing a secret only to certain roles
or depending on the entity).
