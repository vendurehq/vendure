...
A client which sends no `Accept-Language` header behaves as it always has, falling back to `languageCode`.
...
:::caution
Browsers send an `Accept-Language` header of their own, derived from the operating system locale, on every request. A browser-based Admin API client which has been relying on `languageCode` alone for these strings will therefore start resolving them against that OS locale instead.
