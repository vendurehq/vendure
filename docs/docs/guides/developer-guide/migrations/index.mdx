---
title: "Migrations"
metaTitle: "Database Migrations in Vendure - Generate, Run, and Revert"
metaDescription: "Manage database schema changes in Vendure with TypeORM migrations. Covers generating, running, and reverting migrations for custom fields and plugins."
---
...
### Generating a baseline migration for an existing project

`generateMigration` compares your configuration against the schema of the _currently configured_ database. Run it against a database that already contains the schema and it finds no differences, so it cannot produce an initial migration. This is a problem if you have an existing project with no initial migration and need to create one.
