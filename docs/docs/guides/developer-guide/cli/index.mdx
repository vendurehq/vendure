...
The v2 manifest contains identity metadata only:
...
The Console values are origins. They contain no path, query, fragment, or credentials. The CLI also accepts a v1
manifest without `console` and treats it as a production Console link. Running `vendure console link` again upgrades
that file to v2.
...
The command uses the Console recorded in `.vendure/project.json`. A v1 manifest and an unlinked project use
...
`vendure console link` writes or reuses the Project Link Manifest. A repair also adds the Console origins to a legacy
v1 manifest:
