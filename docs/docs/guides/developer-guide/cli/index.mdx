...
project. It is deliberately loose: it separates "not in a project at all" from
"in a project", and does not promise that a given command can succeed. A command
with stricter needs — a config file in the current directory, a particular
package installed — still checks for itself and reports its own error.
...
This is for environments that have no project and no writable home directory —
a container image, or an ephemeral sandbox given to an AI agent. The image
installs the packages and sets the variable, with no setup step to run. Since
it is read afresh at every invocation, `vendure plugins remove --global` cannot
countermand it; unset the variable instead.
