...
When you run `vendure`, the CLI auto-discovers **direct** dependencies that declare
`vendure.cliPlugin`. You can optionally restrict or exclude plugins from the project
`package.json`:
...
A package named in `plugins` must also be declared as a direct project dependency.
A command whose name matches a built-in replaces it (last loaded plugin wins). Plugins
can still call the original via `builtinCommands.<name>.action(...)`.
