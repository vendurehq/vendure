...
metaTitle: "Worker and Job Queue Configuration in Vendure"
metaDescription: "Run background tasks in Vendure using the Worker process and job queue system. Covers setup, job queue strategies, custom job processors, and BullMQ integration."
order: 5
---
...
A deserialized `RequestContext` has no `req`, and its `session` has no `token`: the raw request and the session token are never serialized, so neither is available on the worker. A worker has no session credential by design. Code which calls another system from a job should use a service-level credential.
...
A job which needs values from the request, such as the hostname or a custom header, must copy them into its own job data at the point where the job is added, while the request is available:

```ts
exportAllProducts(ctx: RequestContext) {
    return this.jobQueue.add({
        ctx: ctx.serialize(),
        hostname: ctx.req?.get('host'), // [!code highlight]
    });
}
```
