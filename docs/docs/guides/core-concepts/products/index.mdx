...
Multiple variants are normally defined by adding one or more [`ProductOptionGroups`](/reference/typescript-api/entities/product-option-group) to
...
A product does not have to use option groups. It may hold any number of variants with no options at all. Vendure
does not check that their names or SKUs differ, so give each one a distinct name and SKU yourself.
