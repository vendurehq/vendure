---
title: 'Promotions'
metaTitle: "Creating Promotions and Discounts in Vendure"
metaDescription: "Understand how promotions work in Vendure, including constraints, conditions, and actions for order-level, item-level, and shipping discounts."
---
...
For details on creating custom conditions and actions, see the [Developer Guide](/developer-guide/strategies-configurable-operations/).
...
A Promotion is only recorded on `Order.promotions` — and therefore only consumes its `usageLimit` and `perCustomerUsageLimit` — if it actually adjusted an OrderLine or a ShippingLine. The exception is a Promotion with at least one action defining `onActivate`/`onDeactivate`: the free gift action above returns `0` from `execute()` until its side effect has added the gift line, so such a Promotion is kept whatever it discounts. The presence of a hook, not its body, is what does this: an action defining lifecycle hooks is assumed to deliver its benefit through them rather than through its discount. So if you write a custom action which delivers its benefit by mutating the Order inside `execute()` and returns `0`, an `onActivate` which does nothing is enough to keep the Promotion recorded — add a one-line comment saying it must stay, since it will otherwise read as dead code to the next person.
:::
...
In this scenario, we would have to repeat the logic for checking the Order contents in _both_ the PromotionCondition _and_ the PromotionAction. Instead, we can say that the PromotionAction _depends_ on the PromotionCondition:
