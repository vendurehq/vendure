...
metaTitle: "Horizontal Scaling for Vendure with Docker, Kubernetes, and PM2"
metaDescription: "Configure Vendure for multi-instance deployments using external job queues, cache strategies, and tools like Docker, Kubernetes, or PM2 for horizontal scaling."
order: 2
---
...
* When using cookies to manage sessions, make sure all instances are using the _same_ cookie secret:
    ...
    ```
- Individual Channels and Zone lists use short-lived process-local L1 caches backed by the global CacheStrategy as a shared L2. This avoids loading every Channel or accessing Redis or SQL on every request. L1 entries are eventually consistent between instances and live for 30 seconds by default, configurable via [EntityOptions](/reference/typescript-api/configuration/entity-options/).
