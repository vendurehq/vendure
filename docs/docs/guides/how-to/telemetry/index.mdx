...
If you already have this plugin running, read [Upgrading from earlier
versions](#upgrading-from-earlier-versions) before you deploy. Span attribute names have changed,
which will break existing Grafana dashboards and alerts.
...
The Open Telemetry libraries for Node.js instrument underlying libraries such as HTTP, Express,
NestJS, GraphQL, the PostgreSQL & MySQL2 database drivers, and ioredis to collect telemetry data.
In order to do this, they need to be preloaded before any of the Vendure application code. This
is done by means of a preload script.

The script imports the OpenTelemetry SDK and exporters directly, so install them alongside the
plugin. Pin them to the versions the plugin is built against, otherwise npm resolves the latest
line and you end up with two copies of the SDK in your tree:

```bash
npm install @opentelemetry/exporter-logs-otlp-proto@0.222.0 \
...
```bash
npm install @opentelemetry/auto-instrumentations-node@0.80.0
```
...
const config = getSdkConfiguration({
    config: {
        spanProcessors: [new BatchSpanProcessor(traceExporter)],
        logRecordProcessors: [new BatchLogRecordProcessor({ exporter: logExporter })],
        instrumentations: [getNodeAutoInstrumentations()], // [!code highlight]
    },
});
```
...
```bash
npm install @opentelemetry/instrumentation-http@0.222.0 \
...
const config = getSdkConfiguration({
    config: {
        spanProcessors: [new BatchSpanProcessor(traceExporter)],
        logRecordProcessors: [new BatchLogRecordProcessor({ exporter: logExporter })],
        instrumentations: [ // [!code highlight]
            new HttpInstrumentation(), // [!code highlight]
            new ExpressInstrumentation(), // [!code highlight]
            new NestInstrumentation(), // [!code highlight]
            new GraphQLInstrumentation(), // [!code highlight]
            new PgInstrumentation(), // [!code highlight]
            new MySQL2Instrumentation(), // [!code highlight]
            new IORedisInstrumentation(), // [!code highlight]
            new KafkaJsInstrumentation(), // [!code highlight]
        ],
    },
});
```
...
The plugin now uses the OpenTelemetry 0.222 instrumentations, which emit only the stable semantic
conventions. Two things change in the telemetry your server produces. Both apply whether or not
you customise `instrumentations`.

:::warning[Span attribute names have changed]
The attributes on HTTP and database spans are renamed. Names like `http.method` and
`db.statement` are gone, and you cannot get them back. Grafana dashboards and alerts that query
an old name will come back empty.
...
| `db.user`, `db.connection_string` | removed, no replacement |
| `http.request_content_length`, `http.response_content_length` | removed, no replacement |

Span names and `http.route` are unchanged.

**Migrating your dashboards**

1. Find what breaks. Search your dashboards and alert rules for `http.`, `net.` and `db.`. In
   Grafana, read them from **Dashboard settings > JSON Model**, or fetch them all through the
   HTTP API.
2. Rename the queries using the table above. If you build metrics from spans, Prometheus turns
   the dots into underscores, so `http_method` becomes `http_request_method`.
3. Query both names for now. Traces recorded before the upgrade still carry the old names, so a
   panel asking only for the new name will have a gap. In TraceQL:
   `{span.http.method="GET" || span.http.request.method="GET"}`. Drop the old half once your
   trace retention window has passed.
4. Deploy, then send a request and check each panel before you delete the old queries.

If you cannot change your dashboards yet, put an OpenTelemetry Collector between Vendure and your
tracing backend. A `transform` processor can copy each new attribute onto its old name, so every
span carries both and you can migrate later.
:::

:::warning[Query parameters are now redacted on incoming spans]
Sensitive query parameters are redacted on spans for incoming requests. Earlier versions only
did this for outgoing requests.

If the request URL carries any of `sig`, `Signature`, `AWSAccessKeyId`, `X-Goog-Signature`,
`X-Amz-Signature`, `X-Amz-Credential` or `X-Amz-Security-Token`, the value is replaced with
`REDACTED` in the `url.query` attribute. No other attribute on the span changes, and the request
itself is untouched.

To use a different list, set `redactedQueryParamsServer` on `HttpInstrumentation`. You need to
pass your own `instrumentations` array to do this, as shown in
[Customising instrumentation](#customising-instrumentation).
:::
