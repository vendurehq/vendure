...
:::warning
Install these instrumentations at the versions shown, not at `latest`. From
`@opentelemetry/instrumentation-http@0.221.0` onwards the HTTP instrumentation emits only the
stable semantic conventions, with no opt-out: `http.method` becomes `http.request.method` and
`net.peer.name` becomes `server.address`. Since your `instrumentations` array replaces the
curated default rather than adding to it, a newer package silently renames the attributes on
every HTTP span and breaks any dashboard or alert keyed on the old names.
:::
