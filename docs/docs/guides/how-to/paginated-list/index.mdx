...
and how much more depends on the database.

| Database | Where the pattern runs | Cost per row |
|---|---|---|
| PostgreSQL | In the database | Bounded. The database abandons a pattern which takes too long |
| MySQL 8 | In the database | Bounded. The database abandons a pattern which takes too long |
| MariaDB | In the database | Rises steeply. PCRE2 backtracks, and one pattern can take hundreds of milliseconds against a single long value |
| SQLite, SQL.js | In the Node.js process | Bounded. Vendure matches with RE2, which does not backtrack |

SQLite and SQL.js evaluate the pattern in the Node.js process rather than in the database. Vendure
matches those with the [RE2](https://github.com/google/re2) engine, which runs in linear time, so no
pattern can block the event loop. RE2 supports neither lookaround nor backreferences, and a filter
which uses either is rejected with an error rather than failing part-way through the query.

MariaDB is the one backend with no bound on what a single pattern can cost. It matches with PCRE2,
which backtracks, and it pays that cost for every row it reads. Set a statement timeout on MariaDB
so that one query cannot hold a connection open indefinitely.
