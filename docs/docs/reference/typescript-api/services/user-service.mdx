...
generated: true
---
<GenerationInfo sourceFile="packages/core/src/service/services/user.service.ts" sourceLine="36" packageName="@vendure/core" />

Contains methods relating to [User](/reference/typescript-api/entities/user#user) entities.

```ts title="Signature"
class UserService {
    constructor(connection: TransactionalConnection, configService: ConfigService, passwordCipher: PasswordCipher, verificationTokenGenerator: VerificationTokenGenerator, moduleRef: ModuleRef)
    ...
    createApiKeyUser(ctx: RequestContext, identifier: string) => Promise<User>;
...
Creates a new customer User using the [NativeAuthenticationStrategy](/reference/typescript-api/auth/native-authentication-strategy#nativeauthenticationstrategy). The
...
<MemberInfo kind="method" type={`(ctx: <a href='/reference/typescript-api/request/request-context#requestcontext'>RequestContext</a>, identifier: string) => Promise<<a href='/reference/typescript-api/entities/user#user'>User</a>>`}   />
...
[RoleAssignmentService](/reference/typescript-api/services/role-assignment-service#roleassignmentservice) before a session is created for it.
### softDelete
