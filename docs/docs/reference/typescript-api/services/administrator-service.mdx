...
(see [RoleService](/reference/typescript-api/services/role-service#roleservice)), and so keep gating the Role for live administrators. One
`removed` [RoleAssignmentEvent](/reference/typescript-api/events/event-types#roleassignmentevent) is published for each removed assignment.
