...
<GenerationInfo sourceFile="packages/core/src/event-bus/events/role-assignment-event.ts" sourceLine="28" packageName="@vendure/core" since="4.0.0" />

This event is fired whenever [RoleAssignment](/reference/typescript-api/entities/role-assignment#roleassignment) rows of a [User](/reference/typescript-api/entities/user#user) change, i.e. when
...
```ts title="Signature"
class RoleAssignmentEvent extends VendureEvent {
    constructor(ctx: RequestContext, user: User, assignments: RoleChannelPair[], type: 'assigned' | 'removed')
}
```
* Extends: [`VendureEvent`](/reference/typescript-api/events/vendure-event#vendureevent)
