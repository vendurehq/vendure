...
<GenerationInfo sourceFile="packages/core/src/event-bus/events/administrator-password-reset-event.ts" sourceLine="14" packageName="@vendure/core" since="3.8.0" />

This event is fired when an Administrator requests a password reset email.

```ts title="Signature"
class AdministratorPasswordResetEvent extends VendureEvent {
    constructor(ctx: RequestContext, administrator: Administrator, user: User)
}
```
* Extends: [`VendureEvent`](/reference/typescript-api/events/vendure-event#vendureevent)
