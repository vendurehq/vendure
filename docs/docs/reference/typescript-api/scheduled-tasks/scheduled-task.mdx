...
<GenerationInfo sourceFile="packages/core/src/scheduler/scheduled-task.ts" sourceLine="133" packageName="@vendure/core" since="3.3.0" />
...
class ScheduledTask<C extends Record<string, any> = Record<string, any>> {
    ...
    configure(additionalConfig: Partial<
...
<MemberInfo kind="method" type={`(additionalConfig: Partial<             Pick<<a href='/reference/typescript-api/scheduled-tasks/scheduled-task#scheduledtaskconfig'>ScheduledTaskConfig</a><C>, 'schedule' | 'timezone' | 'timeout' | 'params'>         >) => `}   />
