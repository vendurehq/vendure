...
is defined in [EntityOptions](/reference/typescript-api/configuration/entity-options#entityoptions):
...
The [DefaultMoneyStrategy](/reference/typescript-api/money/default-money-strategy#defaultmoneystrategy) uses an `int` field in the database, which puts an
...
[BigIntMoneyStrategy](/reference/typescript-api/money/big-int-money-strategy#bigintmoneystrategy) which will use the `bigint` type to store monetary values,
