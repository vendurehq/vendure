...
An Order is created whenever a [Customer](/reference/typescript-api/entities/customer#customer) adds an item to the cart. It contains all the
information required to fulfill an order: which [ProductVariant](/reference/typescript-api/entities/product-variant#productvariant)s in what quantities;
the shipping address and price; any applicable promotions; payments etc.

An Order exists in a well-defined state according to the [OrderState](/reference/typescript-api/orders/order-process#orderstate) type. A state machine
...
[OrderCodeStrategy](/reference/typescript-api/orders/order-code-strategy#ordercodestrategy). This should be used as an order reference
...
sum of [OrderLine](/reference/typescript-api/entities/order-line#orderline)'s `discountedLinePrice` values.
### subTotalWithTax
