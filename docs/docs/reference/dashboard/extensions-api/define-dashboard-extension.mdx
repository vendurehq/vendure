...
<MemberInfo kind="property" type={`<a href='/reference/dashboard/extensions-api/navigation#navmenutransform'>NavMenuTransform</a>[]`}  since="3.8.0"  />

Functions which transform the nav menu on every render, with access to the
logged-in user via [DashboardUserContext](/reference/dashboard/extensions-api/navigation#dashboardusercontext). Use this to hide, reorder,
